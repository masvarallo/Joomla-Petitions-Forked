...
class PetitionsViewPetition extends JView
{
	function display( $tpl = null )
	{
		...
		$category	= &$this->get('category' );//milos
		...
		{
			$link	= '&view=petition&id='.$category->slug.'&format=feed&limitstart=';//milos
		...
		//set breadcrumbs
		if(is_object($menu) && $menu->query['view'] != 'petition') {
		...
		$params->def('com_description', JText::_('PETITIONS_DESC'));
		...
		for($i = 0; $i < $count; $i++)
		{
			...
			$link = JRoute::_( 'index.php?view=petition&catid='.$category->slug.'&id='. $item->slug);
